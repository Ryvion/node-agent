...
func Sample() SampleSet {
    cpu := sampleCPUUtil()
    mem := sampleMemUtil()
    if cpu < 0 { cpu = rand.Float64()*70 + 10 }
    if mem < 0 { mem = rand.Float64()*70 + 10 }
    gpu := GPUUtilSnapshot(context.Background())
    if gpu < 0 { gpu = rand.Float64()*70 + 10 }
    return SampleSet{
        CPUUtil:    cpu,
        MemUtil:    mem,
        GPUUtil:    gpu,
...
func sampleCPUUtil() float64 {
    // Linux /proc/stat twice with short delay
...
func parseProcStatCPU(line string) (uint64, uint64) {
    ...
    var vals []uint64
    for i := 1; i < len(f); i++ {
        if v, err := strconv.ParseUint(f[i], 10, 64); err == nil { vals = append(vals, v) }
    }
    if len(vals) < 4 { return 0, 0 }
    user := vals[0]
    nice := uint64(0)
    if len(vals) > 1 { nice = vals[1] }
    system := uint64(0)
    if len(vals) > 2 { system = vals[2] }
    idle := vals[3]
    total := uint64(0)
    for _, v := range vals { total += v }
    used := user + nice + system
    // total used = total - idle
    _ = idle
    return used, total
}
    ...
    if out, err := exec.Command("sh", "-lc", "grep -E 'MemTotal|MemAvailable' /proc/meminfo").CombinedOutput(); err == nil {
        ...
        for _, ln := range lines {
            ...
            if strings.HasPrefix(ln, "MemTotal:") {
                ...
                if len(fields) >= 2 { totalKB, _ = strconv.ParseUint(fields[1], 10, 64) }
            }
            if strings.HasPrefix(ln, "MemAvailable:") {
                fields := strings.Fields(ln)
                if len(fields) >= 2 { availKB, _ = strconv.ParseUint(fields[1], 10, 64) }
            }
        }
...
// --- helpers ---
type gpuInfo struct{ model string; vramBytes uint64; sensors string }
...
func detectGPU() gpuInfo {
    ...
    if err != nil { return gpuInfo{} }
    lines := strings.Split(strings.TrimSpace(string(out)), "\n")
    if len(lines) == 0 { return gpuInfo{} }
    parts := strings.Split(lines[0], ",")
    for i := range parts { parts[i] = strings.TrimSpace(parts[i]) }
    var model, driver string
    var vramMB int64
    if len(parts) > 0 { model = parts[0] }
    if len(parts) > 1 { if v, err := strconv.ParseInt(parts[1], 10, 64); err == nil { vramMB = v } }
...
func detectRAMBytes() uint64 {
    ...
    if out, err := exec.Command("sh", "-lc", "grep -i MemTotal /proc/meminfo | awk '{print $2}'").CombinedOutput(); err == nil {
        ...
        if s != "" { if kb, err := strconv.ParseUint(s, 10, 64); err == nil { return kb * 1024 } }
    }
    // Try macOS sysctl
    if out, err := exec.Command("sh", "-lc", "sysctl -n hw.memsize").CombinedOutput(); err == nil {
        s := strings.TrimSpace(string(out))
        if s != "" { if b, err := strconv.ParseUint(s, 10, 64); err == nil { return b } }
    }
    // Try Windows wmic
    if out, err := exec.Command("wmic", "ComputerSystem", "get", "TotalPhysicalMemory").CombinedOutput(); err == nil {
        lines := strings.Split(strings.TrimSpace(string(out)), "\n")
        if len(lines) >= 2 { if b, err := strconv.ParseUint(strings.TrimSpace(lines[1]), 10, 64); err == nil { return b } }
    }
    // Fallback
