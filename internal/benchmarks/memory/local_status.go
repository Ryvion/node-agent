...
func (s *LocalStatus) RecordRejected(jobID string, err error) {
	s.recordRejectedAt(jobID, err, time.Now())
}
...
func SanitizeLocalReceiptMetadata(metadata map[string]any) map[string]any {
	if metadata == nil {
		return nil
	}
	out := make(map[string]any, len(metadata))
	for key, value := range metadata {
		if key == BenchmarkTask {
			out[key] = sanitizeBenchmarkLocalMetadata(value)
			continue
		}
		out[key] = value
	}
...
func (s *LocalStatus) recordRejectedAt(jobID string, err error, at time.Time) {
	if s == nil {
		return
	}
	s.mu.Lock()
...
func sanitizeBenchmarkLocalMetadata(value any) any {
	benchmark, ok := value.(map[string]any)
	if !ok {
		return value
	}
	out := make(map[string]any, len(benchmark))
	for key, nested := range benchmark {
		if key == "weighted_value" {
			continue
		}
		out[key] = nested
	}
