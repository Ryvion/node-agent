...
- `internal/runner/oci.go` — OCI container execution through the managed runtime wrapper
...
5. Register with hub (retry with exponential backoff — important for Windows where the managed runtime/network may not be ready at boot)
...
- Pull the OCI image before every run (15min timeout). Falls back to the cached image on pull failure.
...
- Registration retries with backoff — critical for Windows where the managed runtime/WSL2/network may start late
...
- Runtime provisioning happens in installer/repair flows; the daemon only reports managed runtime availability at startup
