...
import (
	...
	"github.com/akatosh/node-agent/ui/tray"
	upd "github.com/akatosh/node-agent/internal/update"
)
...
func main() {
	...
	// Load config file if present to override defaults when flags are untouched
	cfg := loadConfig()
	...
	// Optional auto-update loop (env guarded)
...
func autoUpdateLoop() {
	...
	applyMode := strings.ToLower(strings.TrimSpace(os.Getenv("AK_UPDATE_APPLY"))) // "exit" to exit after staging
	if base == "" || pub == "" {
	...
	if v := strings.TrimSpace(os.Getenv("AK_UPDATE_INTERVAL_MIN")); v != "" {
		if n, err := strconv.Atoi(v); err == nil && n > 0 { interval = time.Duration(n) * time.Minute }
	}
	for {
...
func checkAndStageUpdate(baseURL, signerPubHex, applyMode string) error {
	...
	src.Close()
	if err != nil { return fmt.Errorf("read signature: %w", err) }
	...
	if err := upd.VerifySHA256Hex(tee, asset.SHA256); err != nil { return fmt.Errorf("sha256 verify: %w", err) }
	// stage next binary
