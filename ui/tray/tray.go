...
// Controller exposes minimal controls to pause/resume and fetch status.
...
// Start starts a minimal local HTTP status page and API on addr (e.g., ":8088").
// Routes:
//   GET  /           — simple UI
//   GET  /api/status — {paused,last_error,last_heartbeat_ms}
//   POST /api/pause  — pause work loop
//   POST /api/resume — resume work loop
func Start(addr string, ctrl Controller) {
    ...
    mux.HandleFunc("/api/payout", func(w http.ResponseWriter, r *http.Request) {
        ...
        if ctrl.SavePayout == nil { http.Error(w, "not supported", 400); return }
        if err := ctrl.SavePayout(req.Wallet); err != nil { http.Error(w, err.Error(), 500); return }
        w.Header().Set("Content-Type", "application/json")
        _ = json.NewEncoder(w).Encode(map[string]any{"ok": true})
    })
    // Proxy to get currently saved payout from hub for convenience
    mux.HandleFunc("/api/payout/current", func(w http.ResponseWriter, r *http.Request) {
        if ctrl.Hub == "" || ctrl.PubHex == "" { http.Error(w, "missing hub/pubkey", 500); return }
        u, _ := neturl.Parse(ctrl.Hub + "/api/v1/node/payout")
        q := u.Query(); q.Set("pubkey", ctrl.PubHex); u.RawQuery = q.Encode()
